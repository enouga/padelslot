...
> **Évolution (2026-06-11) — planning caisse :** le planning admin passe en **grille verticale** (terrains en colonnes avec prix/h + badge Single, heures à gauche, en-tête et gouttière sticky, barre « maintenant » horizontale, auto-scroll sur l'heure courante le jour J). Panneau Encaisser : **chips de montant** « Reste / Total / ÷ joueur » (÷4 double, ÷2 single via `Resource.attributes.format`) + **boutons moyen 1-clic** (Espèces/Carte/Virement/Ticket CE/Autre — le select et le bouton Encaisser disparaissent ; Ticket CE ouvre la saisie référence/émetteur en 2 temps). Chaque bloc COURT payant affiche des **pastilles de paiement** (1 point plein par paiement sur 4 ou 2 places, tout vert + ✓ si soldé, badge réduit sur les blocs trop petits, `+n` si plus de paiements que de places). Helpers purs en centimes `frontend/lib/caisse.ts` (+ `playerCount` dans `lib/courtType.ts`), composant `frontend/components/admin/PaymentDots.tsx`, tests `frontend/__tests__/caisse.test.ts`.
